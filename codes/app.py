...
def desired_company_mentoring(name, user_desired_company, companies, skills, user_info):
    if user_desired_company in companies:
        ...
        for company in companies[user_desired_company]:
            ...
            company_required_skills = set(company.get('required_skills', []))
            if jaccard_similarity(set(skills), company_required_skills) >= 0.5:
                required_percentage = calc_similarity_percentage(skills, company_required_skills)
                print(f"필수 기술 요구사항에 {user_info['name']}님이 {company['title']}의 지원 자격에 {required_percentage}% 충족합니다.")
                company_preferential_skills = set(company.get('preferential', []))
                if jaccard_similarity(set(skills), company_preferential_skills) >= 0.5:
...
def recommend_company_mentoring(name, companies, skills, user_info):
    ...
    if recommend_company:
        ...
        for company in recommend_company :
            result_text[company] = {"required_score" : "충족하지 않음", "preferential" : "충족하지 않음"}
            ...
            for posting in postings:
                ...
                if jaccard_similarity(set(skills), company_required_skills) >= 0.5:
                    required_percentage = calc_similarity_percentage(skills, company_required_skills)
                    result_text[company]["required_score"] = f"{required_percentage:.1f}%"
                if jaccard_similarity(set(skills), company_preferential_skills) >= 0.5:
                    preferential_percentage = calc_similarity_percentage(skills, company_preferential_skills)
                    result_text[company]["preferential"] = f"{preferential_percentage:.1f}%"
                    result_text[company]["preferential"] = ", ".join(company_preferential_skills)
        return result_text
    else : 
        return None
    """
...
def result(user_form_data):
    ...
    user_info["skills"] = set(user_form_data["skills"].split(", "))
    ...
    if user_info["desired_company"] == '':
        return recommend_company_mentoring(user_info['name'], companies, user_info['skills'], user_info)
    else:
        return desired_company_mentoring(user_info['name'], user_info["desired_company"], companies, user_info["skills"], user_info)
